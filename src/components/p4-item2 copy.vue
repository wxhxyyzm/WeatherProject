<template>
  <!-- 降水信息 -->
  <div id="chartDom2" class="chart"></div>
</template>
    <!-- 折线图 -->
  <script>
import { inject, onMounted } from "vue";
import jsonData from "../assets/guangzhou.json";
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const myChart = $echarts.init(document.getElementById("chartDom2"), roma);
      // 绘制图表
      ...
      const ydata = jsonData.map((item) => item.DEWP); // 平均温度
      const y2data = jsonData.map((item) => item.PRE_DEWP);
      console.log(ydata);
      myChart.setOption({
        title: {
          text: "露点温度与预测露点温度",
