<template>
  <!-- 降水信息 -->
  <div id="chartDom4" class="chart"></div>
</template>
    <!-- 折线图 -->
  <script>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const myChart = $echarts.init(document.getElementById("chartDom4"), roma);
      // 绘制图表
      ...
      const ydata = jsonData.map((item) => item.MIN_TEMPERATURE); // 平均风速
      const y2data = jsonData.map((item) => item.PRE_MIN_TEMPERATURE);
      console.log(ydata);
      myChart.setOption({
        title: {
          text: "日最低温与预测日最低温",
